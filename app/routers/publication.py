...
@router.get("/", summary="Get publications.", response_model=AmbaResponse)
def get_publications_router(
        offset: int = 0, limit: int = 10, sort: str = 'id', order: str = 'asc', search: str = '',
        session: Session = Depends(get_session)
):
    """
    Return publication data. This will **not** contain any trending or processed data.
    ...
    """
    start = time.time()
    json_compatible_item_data = get_publications(session=session, offset=offset, limit=limit, sort=sort, order=order,
                                                 search=search)
